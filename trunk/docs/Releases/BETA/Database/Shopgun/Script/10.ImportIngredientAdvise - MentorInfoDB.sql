...
DECLARE @ingredientsId int, @mentorId int, @semaphoreId int
...
BEGIN
	...
	SELECT @ingredientsId = [Shopgun].[dbo].Ingredients.Id
	FROM [Shopgun].[dbo].Ingredients
	WHERE [Shopgun].[dbo].Ingredients.IngredientName = @splitedRetVal
	
	SELECT @semaphoreId = [Shopgun].[dbo].Semaphore.Id
	FROM [Shopgun].[dbo].Semaphore
	WHERE [Shopgun].[dbo].Semaphore.ColorName = @semaphore
	
	IF NOT (@mentorId IS NULL OR @ingredientsId IS NULL OR @semaphoreId IS NULL)
	BEGIN
		...
		INSERT INTO [Shopgun].[dbo].[Advices]
			   ...
			   ,[Published]
			   ,[PublishDate])
			   VALUES(@ingredientsId,'IngredientAdvice',@mentorId,
						@headline, @introduction, @runnning_text, @search_object, @semaphoreId, 1, @last_updated)
						
	--DEGUG : SELECT @ingredientsId, 'IngredientAdvice', @mentorId, @headline, @introduction, @runnning_text,
