...
DECLARE @countrysId int, @mentorId int, @semaphoreId int
...
BEGIN
	...
	SELECT @countrysId = [Shopgun].[dbo].[Countries].Id
	FROM [Shopgun].[dbo].[Countries] INNER JOIN
	[Shopgun].[dbo].[CountryCodes] ON 
	[Shopgun].[dbo].[Countries].CountryCodeId = [Shopgun].[dbo].[CountryCodes].Id
	WHERE [Shopgun].[dbo].[CountryCodes].Name= @splitedRetVal
	
	SELECT @semaphoreId = [Shopgun].[dbo].Semaphore.Id
	FROM [Shopgun].[dbo].Semaphore
	WHERE [Shopgun].[dbo].Semaphore.ColorName = @semaphore
	
	IF NOT (@mentorId IS NULL OR @countrysId IS NULL OR @semaphoreId IS NULL)
	BEGIN
		...
		INSERT INTO [Shopgun].[dbo].[Advices]
			   ...
			   ,[Published]
			   ,[PublishDate])
			   VALUES(@countrysId,'CountryAdvice',@mentorId,
						@headline, @introduction, @runnning_text, @search_object, @semaphoreId, 1, @last_updated)
					
	--DEGUG : SELECT @countrysId, @splitedRetVal, 'CountryAdvice', @mentorId, @headline, @introduction, @runnning_text,
