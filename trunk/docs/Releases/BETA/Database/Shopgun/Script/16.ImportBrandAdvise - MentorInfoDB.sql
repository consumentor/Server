...
DECLARE @brandsId int, @mentorId int, @semaphoreId int
...
BEGIN
	...
	SELECT @brandsId = [Shopgun].[dbo].[Brands].Id
	FROM [Shopgun].[dbo].[Brands]
	WHERE [Shopgun].[dbo].[Brands].BrandName= @splitedRetVal
	
	SELECT @semaphoreId = [Shopgun].[dbo].Semaphore.Id
	FROM [Shopgun].[dbo].Semaphore
	WHERE [Shopgun].[dbo].Semaphore.ColorName = @semaphore
	
	IF NOT (@mentorId IS NULL OR @brandsId IS NULL OR @semaphoreId IS NULL)
	BEGIN
		...
		INSERT INTO [Shopgun].[dbo].[Advices]
			   ...
			   ,[Published]
			   ,[PublishDate])
			   VALUES(@brandsId,'BrandAdvice',@mentorId,
						@headline, @introduction, @runnning_text, @search_object, @semaphoreId, 1, @last_updated)
						
	--DEGUG : SELECT @brandsId, 'IngredientAdvice', @mentorId, @headline, @introduction, @runnning_text,
