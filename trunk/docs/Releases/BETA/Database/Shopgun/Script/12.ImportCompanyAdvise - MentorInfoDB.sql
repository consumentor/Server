...
DECLARE @companysId int, @mentorId int, @semaphoreId int
...
BEGIN
	...
	SELECT @companysId = [Shopgun].[dbo].Companies.Id
	FROM [Shopgun].[dbo].Companies
	WHERE [Shopgun].[dbo].Companies.Name = @splitedRetVal
	
	SELECT @semaphoreId = [Shopgun].[dbo].Semaphore.Id
	FROM [Shopgun].[dbo].Semaphore
	WHERE [Shopgun].[dbo].Semaphore.ColorName = @semaphore
	
	IF NOT (@mentorId IS NULL OR @companysId IS NULL OR @semaphoreId IS NULL)
	BEGIN
		...
		INSERT INTO [Shopgun].[dbo].[Advices]
			   ...
			   ,[Published]
			   ,[PublishDate])
			   VALUES(@companysId,'CompanyAdvice',@mentorId,
						@headline, @introduction, @runnning_text, @search_object, @semaphoreId, 1, @last_updated)
						
	--DEGUG : SELECT @companysId, 'IngredientAdvice', @mentorId, @headline, @introduction, @runnning_text,
