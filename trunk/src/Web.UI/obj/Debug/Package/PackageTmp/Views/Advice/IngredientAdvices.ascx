...
<script type="text/javascript">
    ...
    function ingredientAdvicesFormatter(cellvalue) {
        semaphores = {};
    ...
    function ingredientAdvices(cellvalue) {
        signals = [];
        for (var i in cellvalue) {
    ...
    jQuery(document).ready(function fillList() {
        jQuery("#list").jqGrid(
            {
                url: '/IngredientAdvice/IngredientAdvices',
                datatype: 'json',
                mtype: 'POST',
                colNames: ['Id', 'Ingredient', 'Signals', 'Actions'],
                //colNames: ['Ingredient'],
                colModel: [
                    { name: 'Id', index: 'Id', hidden: true },
                    { name: 'IngredientName', index: 'IngredientName', align: 'left' },
                    { name: 'IngredientAdvices', index: 'IngredientAdvices', align: 'left', formatter: ingredientAdvices },
                    { name: "Actions", index: "Actions", formatter: ingredientActionsFormatter }
                ],
                jsonReader: {
                    repeatitems: false,
                    id: "0"
                ...
                subGridRowExpanded: function(subgrid_id, row_id) {
                    ...
                    jQuery("#" + subgrid_table_id).jqGrid(
                        {
                            url: "/Advice/AdvicesForIngredient/" + row_id,
                            datatype: "json",
        ...
        function adviceActionsFormatter(cellvalue, options, rowObject) {
            ...
            var actions = "<a href='/IngredientAdvice/Edit/" + id + "'>Edit</a> ";
        ...
        function ingredientActionsFormatter(cellvalue, options, rowObject) {
            id = rowObject.Id;
            //console.log(rowObject);
            var actions = "<a href='/IngredientAdvice/Create/" + id + "'>Add advice</a> ";
            return actions;
        }
    ...
    function refresh() {
        $("#ui-tabs-1").load("/Advice/IngredientAdvices");
    };
...
<p>
    <%= Html.ActionLink("Create New", "Create", "IngredientAdvice") %>
</p>
